...
class StrLabelConverter(object):
    ...
    def __init__(self, alphabet, ignore_case=False, max_text_len=25):
        ...
        self.alphabet = alphabet + 'ό'  # for `-1` index
        self.dict = {}
        for i, char in enumerate(alphabet):
            # NOTE: 0 is reserved for 'blank' required by wrap_ctc
            self.dict[char] = i + 1
        ###(self.dict,len(self.dict) + 1)
    ...
    def encode(self, text):
        ...
        if self.max_text_len == 26:
            text += [self.nClasses]
            text = text + [0] * (self.max_text_len - 1 - length)
        else:
            text = text + [0] * (self.max_text_len - length)

        return text[:self.max_text_len], length
    ...
    def decode(self, t, length, scores=None, raw=False):
        ...
        if length.numel() == 1:
            ...
            if raw:
                ...
                temp_score = []
                import numpy as np
                all_scores = np.zeros((self.max_text_len, len(self.alphabet)))  # np.zeros((25, 37))
                t_length = 0
